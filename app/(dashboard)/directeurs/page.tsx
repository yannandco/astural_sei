...
interface Directeur {
  ...
  phone: string | null
  isActive: boolean
  ecoles: DirecteurEcole[]
}

export default function DirecteursPage() {
  const router = useRouter()
  const [items, setItems] = useState<Directeur[]>([])
  ...
  const fetchItems = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/directeurs?${params.toString()}`)
  ...
  return (
    <div>
      <div className="ds-header">
        <div className="ds-header-content">
          <div className="ds-header-left">
            ...
            <div>
              <h1 className="ds-header-title">Directeurs</h1>
              <p className="ds-header-subtitle">{items.length} directeur{items.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
          <Link href="/directeurs/new" className="btn btn-primary">
      ...
      {loading ? (
        ...
        <div className="ds-empty-state"><div className="ds-empty-state-content"><div className="ds-empty-state-icon-wrapper"><UserIcon className="ds-empty-state-icon" /></div><h3 className="ds-empty-state-title">Aucun directeur</h3><p className="ds-empty-state-text">Commencez par ajouter un directeur.</p></div></div>
      ) : (
